...
Variadic Generics is a major feature listed in the [Generics Manifesto](https://github.com/apple/swift/blob/master/docs/GenericsManifesto.md) as one of the major features that needs to be implemented to empower the Swift generics system. It's also a feature that has come up in various threads on the [Swift Forums](https://forums.swift.org), like for example (in no particular order) in [[1]](https://forums.swift.org/t/emulating-variadic-generics-in-swift/20046), [[2]](https://forums.swift.org/t/map-sorting/21421/12), [[3]](https://forums.swift.org/t/variadic-generics/20320), [[4]](https://forums.swift.org/t/implementing-expressiblebytupleliteral-how-hard/21169), [[5]](https://forums.swift.org/t/pitch-function-builders/25167), [[6]](https://forums.swift.org/t/three-steps-to-variadic-generics/33154).

Variadic Generics (*VGs* from now on) allows developers to write generic code that accepts a variable number of types as argument, all optionally conforming to a set of protocols, without the need to write **n** duplicate definitions where n is an hardcoded, maximum supported multiplicity.

Swift-evolution thread: [[Pitch #2] Variadic Generics](https://forums.swift.org/t/variadic-generics-take-two/12345)

Previous thread (pitch #1): [Variadic Generics](https://forums.swift.org/t/variadic-generics/20320)

## Motivation
<!---    1         2         3         4         5         6         7      --->
<!---67890123456789012345678901234567890123456789012345678901234567890123456--->

Today it is impossible or very difficult to express patterns related to an *unbounded amount* of generic types and values. The closest feature that Swift offers as of today is *variadic functions*, but unfortunately such functions require all of their arguments to be of the *same concrete type*:
```swift
func variadicFn<Values: SomeProto>(values: Values...) { }

// This will raise a compile-time error if the concrete type of `a`, `b` and `c`
//  is different, even if all those types conform to `SomeProto`.
variadicFn(a, b, c)
```
This means that in order to create functions (or types) that are generic over a different number of concrete types one must declare such types one by one, and can't even assign them the same name:
```swift
struct GenericOfTwo<A: SomeProto, B: SomeProto>
  where /* the same set of constraints for A and B */ {
...
struct GenericOfThree<A: SomeProto, B: SomeProto, C: SomeProto>
  where /* the same set of constraints for A, B and C*/ {
...
Let's take a look at some examples that are hard or impossible to implement today with regular Swift.
...
func zip<Sequence1: Sequence, Sequence2: Sequence>(
 ...
 ```
The problem here is that this function can only zip *two* sequences, and has to return a specific `Zip2Sequence` type that holds said two sequences. Users who want to zip three sequences would have to create a new `Zip3Sequence` type and overload `zip`, and this leads to unnecessary code duplication:
```swift
struct Zip3Sequence<
  Sequence1: Sequence, 
  Sequence2: Sequence, 
